...
import com.fumiao.pay.mvp.home.MonthBillReportPresenter;
import com.fumiao.pay.mvp.home.MonthBillReportView;
import com.fumiao.pay.tools.DecimalFormatter;
import com.fumiao.pay.tools.MyBarDataSet;
import com.fumiao.pay.ui.activity.home.SelectMonthReportActivity;
...
public class MonthBillReportFragment extends MvpFragment<MonthBillReportPresenter> implements MonthBillReportView {
    ...
    private void init() {
        ...
        mvpPresenter.getMonthBillReport(store_id, "");
        refreshLayout.setEnableLoadMore(false);
        refreshLayout.setOnRefreshListener(new OnRefreshListener() {
            @Override
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                mvpPresenter.getMonthBillReport(store_id, search_month);
            }
        });

        initBarChartView(trendBarChart, 30, 8);
...
//        leftAxis.setAxisMinimum(0f);//不设置会有两条Y线情况
    ...
    @Override
    protected MonthBillReportPresenter createPresenter() {
        return new MonthBillReportPresenter(this, getActivity());
    }

    private float totalAmount = 0; //当前日期交易总和
    private float compareAmount = 0; //对比日期交易总和
    private String currentMonth; //当前月份
    private String lastMonth; //上个月份
    private String searchDay;
    private String searchMonth;

    @Override
    public void showMonthBillReport(MonthBillReportBean reportBean) {
        refreshLayout.finishRefresh();  //刷新完成
        searchMonth = reportBean.getSearch_month();
        billDate.setText(searchMonth.substring(0, 4) + "年" + searchMonth.substring(4) + "月");
        ...
        currentMonth = searchMonth.substring(4) + "月";
        highAmountTime.setText(currentMonth + "收入最高的一天是");
        String compareMonth = reportBean.getCompared_sum().getMonth();
        lastMonth = compareMonth.substring(4) + "月";
        ...
        if (totalAmount >= compareAmount) {
            ...
            amountCompareDetail.setText(currentMonth + "比" + lastMonth + "增加");
            ...
            float differAmount = totalAmount - compareAmount;
            amountCompareDetail.setText(currentMonth + "比" + lastMonth + "减少");
    ...
    private void setCompareBarChartData() {
        ...
        xCompareAxisValue.add(lastMonth);
        xCompareAxisValue.add(currentMonth);
    ...
    private void setTrendBarChartData(List<MonthBillReportBean.TrendSumBean> trendBeans) {
        if (trendBeans != null) {
            ArrayList<BarEntry> yVals1 = new ArrayList<BarEntry>();
            maxVal = 0;
            xAxisValue.clear();
            for (int i = 0; i < trendBeans.size(); i++) {
                ...
                xAxisValue.add(trendBeans.get(i).getDay()+"日");
            }
            ...
            highAmountDetail.setText(day + "日 " + floatToString(maxVal) + "元");
    ...
    private void setPieChartData(List<MonthBillReportBean.PaytypeBean> paytypeBeans) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == SELECT_CANLENDA) {
            search_month = data.getStringExtra("date");
            mvpPresenter.getMonthBillReport(store_id, search_month);
        }
    }
    ...
    public void onViewClicked() {
        ...
        jumpActivityForResult(SelectMonthReportActivity.class, bundle, SELECT_CANLENDA);
    }
