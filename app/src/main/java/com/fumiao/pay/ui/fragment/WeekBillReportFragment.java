...
import com.fumiao.pay.mvp.home.WeekBillReportPresenter;
import com.fumiao.pay.mvp.home.WeekBillReportView;
import com.fumiao.pay.tools.DecimalFormatter;
import com.fumiao.pay.tools.MyBarDataSet;
import com.fumiao.pay.ui.activity.home.SelectWeekReportActivity;
...
public class WeekBillReportFragment extends MvpFragment<WeekBillReportPresenter> implements WeekBillReportView {
    Unbinder unbinder;
    ...
    private void init() {
        ...
        mvpPresenter.getWeekBillReport(store_id, "", "");
        refreshLayout.setEnableLoadMore(false);
        refreshLayout.setOnRefreshListener(new OnRefreshListener() {
            @Override
            public void onRefresh(@NonNull RefreshLayout refreshLayout) {
                mvpPresenter.getWeekBillReport(store_id, search_month, search_week);
            }
        });
        initBarChartView(trendBarChart, 7, 7);
...
//        leftAxis.setAxisMinimum(0f);//不设置会有两条Y线情况
    ...
    private void setCompareBarChartData() {
        ...
        xCompareAxisValue.add(lastWeek);
        xCompareAxisValue.add(currentWeek);
    ...
    private void setTrendBarChartData(List<WeekBillReportBean.TrendSumBean> trendBeans) {
        if (trendBeans != null) {
            ArrayList<BarEntry> yVals1 = new ArrayList<BarEntry>();
            maxVal = 0;
            xAxisValue.clear();
            for (int i = 0; i < trendBeans.size(); i++) {
                ...
                if (val > maxVal) {
                    day = trendBeans.get(i).getZh_week_day();
                    maxVal = val;
                }
                if(val < mimVal){
                    mimVal = val;
                ...
                xAxisValue.add(trendBeans.get(i).getZh_week_day());
            }
            ...
            highAmountDetail.setText(day + " " + floatToString(maxVal) + "元");
    ...
    private void setPieChartData(List<WeekBillReportBean.PaytypeBean> paytypeBeans) {
    ...
    @Override
    protected WeekBillReportPresenter createPresenter() {
        return new WeekBillReportPresenter(this, getActivity());
    }

    @Override
    public void showWeekBillReport(WeekBillReportBean reportBean) {
        refreshLayout.finishRefresh();  //刷新完成
        searchMonth = reportBean.getSearch_month();
        billDate.setText(searchMonth.substring(0, 4) + "年" + searchMonth.substring(4) + "月第" + reportBean.getSearch_week() + "周");
        ...
        currentWeek = searchMonth.substring(4) + "月第" + reportBean.getSearch_week() + "周";
        highAmountTime.setText(currentWeek + "收入最高的一天是");
        String compareWeek = reportBean.getCompared_sum().getWeek_month();
        lastWeek = compareWeek.substring(4) + "月第" + reportBean.getCompared_sum().getWeek() + "周";
        ...
        if (totalAmount >= compareAmount) {
            ...
            amountCompareDetail.setText(currentWeek + "比" + lastWeek + "增加");
            ...
            float differAmount = totalAmount - compareAmount;
            amountCompareDetail.setText(currentWeek + "比" + lastWeek + "减少");
    ...
    public void onViewClicked() {
        ...
        jumpActivityForResult(SelectWeekReportActivity.class, bundle, SELECT_CANLENDA);
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == SELECT_CANLENDA) {
            search_month = data.getStringExtra("search_month");
            search_week = data.getStringExtra("search_week");
            mvpPresenter.getWeekBillReport(store_id, search_month, search_week);
        }
    }
